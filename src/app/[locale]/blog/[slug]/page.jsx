...
const iconOptions = {
  Development: WebDevelopmentIcon,
...
export async function generateMetadata({ params }) {
  ...
  const post = allPosts.find((p) => p.slug === slug && p.language === locale)

  if (!post) return {}

  // Build alternates object for hreflang tags
  const languages = {}
  if (post.alternates) {
    Object.entries(post.alternates).forEach(([lang, altSlug]) => {
      languages[lang] = `https://davidbonan.com/${lang}/blog/${altSlug}`
    })
  }
  ...
  return {
    ...
    alternates: {
      canonical: `https://davidbonan.com/${locale}/blog/${slug}`,
      languages: languages,
    },
    openGraph: {
      title: post.title,
      description: post.description,
      url: `https://davidbonan.com/${locale}/blog/${slug}`,
...
export default function BlogPost({ params }) {
  ...
  // Find adjacent posts for navigation (only in same language)
  const postsInLanguage = allPosts.filter((p) => p.language === locale)
  const sortedPosts = [...postsInLanguage].sort(
