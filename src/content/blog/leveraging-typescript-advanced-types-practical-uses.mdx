...
## Built-in Utility Types: Hidden Gems

TypeScript comes with powerful built-in utility types that many developers overlook. Here are the most practical ones we use daily:
...
// User registration payload (pick only required fields)
type UserRegistration = Pick<User, 'email' | 'password' | 'firstName' | 'lastName'>

// User update payload (make fields optional)
...
```typescript
// Instead of any or unknown for dynamic objects
type APIEndpoints = Record<string, { method: 'GET' | 'POST' | 'PUT' | 'DELETE'; path: string }>
...
```typescript
// Extract return type from existing functions
...
// Useful for creating type-safe wrappers
function loggedFetchUserData(...args: Parameters<typeof fetchUserData>): ReturnType<typeof fetchUserData> {
...
// For complex validation, consider using libraries like Zod
import { z } from 'zod'
